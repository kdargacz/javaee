...
public class DataServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// ang. Stock Keeping Unit) wartości obiekt drzewo w kontekście aplikacji
		ServletContext servletContext = getServletContext();
		StorageService storage = (StorageService) servletContext.getAttribute("storage");
		Tree tree = new Tree(treeName, isLeafy, dataSiewu, cena, qty);
		storage.add(sku, tree);
		...
		out.println("<html><head><link rel='stylesheet' type='text/css' href='" + request.getContextPath()
				...
				+ "<tr><td>" + sku + "</td><td>" + db.get(sku).getName() + "</td><td>" + db.get(sku).isLeafy()
				+ "</td><td>" + db.get(sku).getDataSiewu() + "</td><td>" + db.get(sku).getCena() + "</td><td>"
